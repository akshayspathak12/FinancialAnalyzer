...
public class UserServiceImpl implements UserService {
	...
	public TransactionDetail saveData(TransactionDetailDto detail) throws Exception {
		if (detail.getCustomer_Id() == 0 || detail.getTransaction_Description() == "" || detail.getAmount() == 0
				|| detail.getPayment_Type() == "" || detail.getCategory() == "") {
			LOGGER.info("********** Somthing went wrong *************");
			throw new Exception("enter valid details");
		}
		TransactionDetail save = transactionRepo
				.save(new TransactionDetail(detail.getCustomer_Id(), detail.getTransaction_Description(),
						detail.getAmount(), detail.getPayment_Type(), detail.getCategory(), detail.getDate()));
		return save;
	}

	// provides the list of transactions
	@Override
	public List<TransactionDetailDto> findAllData() throws TransactionsNotFoundException {
		...
		for (TransactionDetail transactionDetail : findAll) {
			list.add(new TransactionDetailDto(transactionDetail.getCustomer_Id(),
					transactionDetail.getTransaction_Description(), transactionDetail.getAmount(),
					transactionDetail.getPayment_Type(), transactionDetail.getCategory(), transactionDetail.getDate()));
	...
	public MonthlySpend saveMonthlyData(MonthlySpendDto detail) throws Exception {
		if (detail.getTotal_Incoming() == 0.0 || detail.getCustomer_Number() == 0.0 || detail.getTotal_Outgoing() == 0.0
				|| detail.getClosing_balance() == 0.0 || detail.getMonth() == "") {
			LOGGER.info("********** Somthing went wrong *************");
			throw new Exception("enter valid details");
		} else {
			MonthlySpend save = monthlySpendRepo.save(new MonthlySpend(detail.getMonth(), detail.getCustomer_Number(),
					detail.getTotal_Incoming(), detail.getTotal_Outgoing(), detail.getClosing_balance()));
			return save;
		}
	}
	...
	public List<TransactionDetailDto> findIncomeData(String paymentCategory) {
		...
		for (TransactionDetail transactionDetail : findIncome) {
			if (transactionDetail.getPayment_Type().equals("credit")) {
				list.add(new TransactionDetailDto(transactionDetail.getCustomer_Id(),
						transactionDetail.getTransaction_Description(), transactionDetail.getAmount(),
						transactionDetail.getPayment_Type(), transactionDetail.getCategory(),
						transactionDetail.getDate()));
			}
		}
		if (list.isEmpty()) {
			LOGGER.info("******** No Transaction Available for " + paymentCategory + " **********");
	...
	public List<MonthlySpendDto> findAllMonthlyData() {
		...
		for (MonthlySpend monthlyDetail : findAll) {
			list.add(new MonthlySpendDto(monthlyDetail.getMonth(), monthlyDetail.getCustomer_Number(),
					monthlyDetail.getTotal_Incoming(), monthlyDetail.getTotal_Outgoing(),
					monthlyDetail.getClosing_balance()));
		}
		if (list.isEmpty()) {
			LOGGER.info("******** No Monthly Transaction Available somthing went wrong  **********");
